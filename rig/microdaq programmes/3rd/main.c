...
volatile unsigned int ReceivedChar[24];	// input from IMU are saved in these variables
...
*/
/*	
	if(flag_0 == 0) // all data is received...
	//put any processing to be done here before transmiting next
	//request
	{	
		//error = 0; now replaced with ReceivedChar[23]
		ReceivedChar[23] = 0;
		U1TXREG = '1' ; // Transmit request to send data		
		flag_0 =1;
		counter = DATA_SIZE;
	...
	{
		//error = 1; now replaced with ReceivedChar[23]
		ReceivedChar[23] = 1;
		U1TXREG = '1' ; // Transmit request to send data		
		flag_0 =1;
		counter = DATA_SIZE;
